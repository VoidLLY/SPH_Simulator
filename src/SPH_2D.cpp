...
{
	// Vector arrays
	x[0][0] = 0.;						// initiate x component of location
	x[1][0] = 0.;						// initiate y component of location
	v[0][0] = 0.;						// initiate x component of velocity
	v[1][0] = 0.;						// initiate y component of velocity
	rho[0] = 1000.;
	...
	P = this->get_P(rho[step]);					//density and pressure
}
...
void SPH_particle::apply_boundary_forces(string keyword)
{
	...
	this->dv[1] -= main_data->g;						// force of gravity
	double h_dash = main_data->max_x[1] / 5;			// the hydraulic head (height of fluid divided by 10, empirically chosen)
	double P = this->rho0 * main_data->g * h_dash;		// pressure at a certain hydraulic head level is calculated by P = phg
	double n1 = 4;										// the power (n) for the repulsive force function
	double cut_off = 0.5 * main_data->dx;				// cut-off distance - if particle is within this cut-off distance from the boundary particle, then repulsive force will occur
...
// Function to update the properties of the particles after every iteration
...
void SPH_main::smoothing_neighbour_itr(SPH_particle* part)
{
	...
	double dist;			// Euclidean distance between particles
	double dn[2];			// array to hold the x and y distances between particles
	double W_sum = 0.;
	double W_sum_rho = 0.;
	int nb_neighbour = 0;   // neighbour count - if particle has no neighbours within 2h, then no need to smooth density
...
void SPH_main::set_values(void)
{
	...
	max_x[0] = 20.0; // Will be 20m wide
	max_x[1] = 10.0; // Will be 10m in height
