...
double get_density(SPH_particle p) {
    /* Return pressure for density p */
    return p.rho[0];
}

double get_boundary(SPH_particle p) {
    /* Return boundary for particle p */
    if (p.boundary)
        return 1;
    else
        return 0;
...
int write_file(const char* filename,
    ...
    fs << scalar_to_string("Boundary", particle_list, get_boundary); //boundary
    fs << "</PointData>\n";
    fs << "<Points>\n";
